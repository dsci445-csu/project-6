...
- Linear Discriminant Analysis (LDA) — A generative model that assumes equal covariance matrices for each class and normally distributed predictors. LDA often performs well in structured tabular data.

- Quadratic Discriminant Analysis (QDA) — Similar to LDA, but allows each class to have its own covariance matrix, giving the model more flexibility.
...
- Hyperparameter tuning for KNN (choice of k), QDA (regularization), and boosting (learning rate, number of trees)
