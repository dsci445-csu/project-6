...
```{r imbalance-plot, echo=FALSE, fig.align="center", out.width="75%"}
ggplot(bank_full, aes(x = y, fill = y)) +
  geom_bar(color = "white") +
  scale_fill_manual(values = c("no" = "grey60", "yes" = "forestgreen")) +
  labs(
...
```{r contact-plot, echo=FALSE, fig.align="center", out.width="75%"}
...
```{r poutcome-plot, echo=FALSE, fig.align="center", out.width="70%"}
ggplot(bank_full, aes(x = poutcome, fill = y)) +
  geom_bar(position = "fill", color = "white") +
  scale_fill_manual(values = c("no" = "grey60", "yes" = "forestgreen")) +
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Subscription Rate by Previous Outcome",
    x = "Previous Outcome",
    y = "Proportion (Yes)"
  ) +
  theme_minimal(base_size = 14)
...
```{r campaign-plot, echo=FALSE, fig.align="center", out.width="75%"}
bank_full %>%
  group_by(campaign) %>%
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Success Rate vs. Number of Campaign Contacts",
...
```{r residuals_slide, echo=FALSE, fig.align="center", results='asis'}
...
```{r knn-prep, echo=FALSE, message=FALSE, fig.align="center", warning=FALSE}
library(dplyr)
library(class)
library(caret)
...
```{r knn-tuning, echo=FALSE, message=FALSE, fig.align="center", warning=FALSE, cache=TRUE}
set.seed(445)
...
## KNN: Evaluation & Test Performance
...
```{r knn-results, echo=FALSE, message=FALSE, fig.align="center",  out.width="75%", warning=FALSE}
library(pROC)
...
# 3. Boosting Tress Model 

```{r boosting-setup, echo=FALSE, fig.align="center", message=FALSE, warning=FALSE}
library(gbm)
library(pROC)
library(caret)
...
```{r boosting-viz, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", out.width="80%"}
plot(
...
```{r boosting-tuning, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", out.width="75%", cache=TRUE}
...
## Boosting: Evaluation & Importance 
...
```{r final-roc, echo=FALSE, message=FALSE, fig.align="center", warning=FALSE}
roc_all <- rbind(
